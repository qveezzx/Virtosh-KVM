...
Please Note that this updater script Will NOT work outside of this project,
it was made and tweaked specifically ONLY for THIS project.

It CAN be used manually from the root Virtosh repo folder.
...
#print("wget -q --output-document=./resources/.webversion --no-cache --no-cookies https://raw.githubusercontent.com/qveezzx/Virtosh-KVM/"+str(targetBranch)+"/.version")

if integrity == 1:
   if os.path.exists("./resources/.webversion"): os.system("rm ./resources/.webversion")
   #os.system("wget -q --output-document=./resources/.webversion --no-cache --no-cookies https://raw.githubusercontent.com/qveezzx/Virtosh-KVM/"+str(targetBranch)+"/VERSION")
   os.system("wget -q --output-document=./resources/.webversion --no-cache --no-cookies https://raw.githubusercontent.com/qveezzx/Virtosh-KVM/"+str(targetBranch)+"/.version")
   if os.path.exists("./resources/.upgrade"): os.system("rm ./resources/.upgrade")
   os.system("wget -q --output-document=./resources/.upgrade --no-cache --no-cookies https://raw.githubusercontent.com/qveezzx/Virtosh-KVM/"+str(targetBranch)+"/resources/.upgrade")
   ...
   def updateBrains():
      ...
      if detectChoice1 == "1" and noDelta == 0:
         ...
         if versionNew == webVersion:
            ...
            if menuFlow == 1:
               ...
               if detectChoice3 == "W" or detectChoice3 == "w":
                  ...
                  print("\n   Opening the release notes in\n   your default browser. Please be patient.\n\n   You will be returned to the main menu in 5 seconds.\n\n\n\n\n")
                  os.system('xdg-open https://github.com/qveezzx/Virtosh-KVM/blob/'+str(targetBranch)+"/docs/changelogs/v"+versionDash+".md > /dev/null 2>&1")
                  time.sleep(6)
                  clear()
                  os.system('./main.py')
         ...
         if args.version is not None:
            ...
            os.system("git clone -q -b v"+webVersion+" --depth 1 https://github.com/qveezzx/Virtosh-KVM ./updates/"+webVersion+"/ > /dev/null 2>&1")         
         else:
            os.system("git clone -q --depth 1 https://github.com/qveezzx/Virtosh-KVM ./updates/"+webVersion+"/")
         clear()
         clear()
         print("\n\n   "+color.BOLD+color.BLUE+"➔  DOWNLOADING UPDATE..."+color.END,"")
         print("   Download to update folder only\n")
   ...
   if versionInt == webVersionInt and args.force is not True and args.version is None:
      ...
      if menuFlow == 1:
         ...
         print(color.END+"         Return to the Virtosh main menu\n")
   ...
   #https://raw.githubusercontent.com/qveezzx/Virtosh-KVM/"+str(targetBranch)+"/.version
