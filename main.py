...
"""
This script was created by qveezzx as part of the Virtosh-KVM project.
...
class GradientColors:
    """ANSI RGB color support with fallback to 256-color"""
    ...
    def supports_truecolor():
        ...
        return 'truecolor' in colorterm or '24bit' in colorterm or 'xterm-256' in term
    
    @staticmethod
    def rgb(r, g, b):
        """Generate RGB color code (with fallback to 256-color)"""
        if GradientColors.supports_truecolor():
            return f"\033[38;2;{r};{g};{b}m"
        else:
            return f"\033[38;5;{GradientColors.rgb_to_256(r, g, b)}m"
...
def get_gradient_color(position, offset=0):
    """Get purple-pink gradient color at position (0.0 to 1.0)"""
    pos = (position + offset) % 1.0
    ...
    return interpolate_color(colors[segment], colors[segment + 1], local_pos)

def colorize_line(line, line_index, total_lines, offset=0):
    """Apply gradient color to a line of text"""
    position = line_index / max(total_lines - 1, 1)
    r, g, b = get_gradient_color(position, offset)
    return f"{GradientColors.rgb(r, g, b)}{line}{GradientColors.RESET}"
# ============ END GRADIENT COLOR SYSTEM ============
...
def print_ascii_header():
    """Print ASCII art header with smooth gradient animation (no flicker)"""
    ...
    # Build static info lines
    info_lines = [
        "",
        f"    {{color}}{GradientColors.BOLD}VirTosh - KVM Based Hackintosh{GradientColors.RESET} {GradientColors.GRAY}v{version}{GradientColors.RESET}",
        f"    {GradientColors.GRAY}by qveezzx{GradientColors.RESET}",
        "",
        f"    {GradientColors.GRAY}═══════════════════════════════════════════{GradientColors.RESET}",
        f"    {{color}}System:{GradientColors.RESET}  {distro_name}",
        f"    {{color}}Kernel:{GradientColors.RESET}  {kernel_ver}",
        f"    {GradientColors.GRAY}═══════════════════════════════════════════{GradientColors.RESET}",
    ]
    ...
    try:
        ...
        for cycle in range(10):
            ...
            # Build the entire output as a single string
            output = []
            
            # Add ASCII art with gradient
            for i, line in enumerate(ascii_lines):
                colored_line = colorize_line(line, i, total_lines, offset)
                output.append(colored_line)
            
            # Add info with colors
            title_color = get_gradient_color(0.5, offset)
            system_color = get_gradient_color(0.3, offset)
            
            for line in info_lines:
                if '{color}' in line:
                    # Use title color for title line, system color for System/Kernel
                    if 'VirTosh' in line:
                        output.append(line.replace('{color}', GradientColors.rgb(*title_color)))
                    else:
                        output.append(line.replace('{color}', GradientColors.rgb(*system_color)))
                else:
                    output.append(line)
            ...
            time.sleep(0.1)  # Smooth animation speed
        ...
        # Fallback: just print static gradient if animation fails
        for i, line in enumerate(ascii_lines):
            colored_line = colorize_line(line, i, total_lines, 0)
            print(colored_line)
        
        title_color = get_gradient_color(0.5, 0)
        system_color = get_gradient_color(0.3, 0)
        
        print(f"\n    {GradientColors.rgb(*title_color)}{GradientColors.BOLD}VirTosh - KVM Based Hackintosh{GradientColors.RESET} {GradientColors.GRAY}v{version}{GradientColors.RESET}")
        print(f"    {GradientColors.GRAY}by qveezzx{GradientColors.RESET}")
        print(f"\n    {GradientColors.GRAY}═══════════════════════════════════════════{GradientColors.RESET}")
        print(f"    {GradientColors.rgb(*system_color)}System:{GradientColors.RESET}  {distro_name}")
        print(f"    {GradientColors.rgb(*system_color)}Kernel:{GradientColors.RESET}  {kernel_ver}")
        print(f"    {GradientColors.GRAY}═══════════════════════════════════════════{GradientColors.RESET}")
...
def startup():
    ...
    def fts():
        clear()
        print("\n\n   "+color.BOLD+color.BLUE+"                WELCOME TO"+color.CYAN+" VirTosh"+color.BLUE+color.BOLD+""+color.END)
        print("                First time setup wizard\n")
        print("   Welcome! As this is your first time using Virtosh, you\n   must select how you'd like to use the project.\n   The project can be run in 2 modes.")
    ...
    if os.path.exists("blobs/user/USR_CFG.apb"):
            print(color.BOLD+"\n\n  "+color.CYAN,"Virtosh"+color.END+color.GRAY,"v"+version+color.END)
            print("   by qveezzx"+color.END)
            tainted = 1
    else:
        print(color.BOLD+"\n\n   Welcome to"+color.CYAN,"Virtosh"+color.END+color.GRAY,"v"+version+color.END)
        print("   by qveezzx"+color.END)
...
if detectChoice == "1":
...
elif detectChoice == "o" or detectChoice == "O":
    clear()
    print(color.BOLD+color.BLUE+"\n\n   OTHER TOOLS"+color.END)
    print("   Additional utilities and tools:\n")
    print(color.BOLD+"      2. Download macOS...")
    print(color.END+"         Download and convert macOS images\n")
    print(color.BOLD+"      3. Compatibility checks...")
    print(color.END+"         Check GPU compatibility and system requirements\n")
    print(color.BOLD+"      4. Passthrough tools...")
    print(color.END+"         VFIO-PCI configuration and device passthrough\n")
    print(color.END+"      B. Back...")
    print(color.END+"      Q. Exit\n")
    otherChoice = str(input(color.BOLD+"Select> "+color.END))
    ...
    elif otherChoice == "b" or otherChoice == "B":
        os.system('./main.py --skip-vm-check')
    elif otherChoice == "q" or otherChoice == "Q":
        exit(0)
elif detectChoice == "2":
    log_info("Opening macOS download tool...")
    os.system('./scripts/dlosx.py')
    ...
    os.system('xdg-open https://github.com/qveezzx/Virtosh-KVM/blob/main/docs/changelogs/v'+versionDash+".md > /dev/null 2>&1")
    time.sleep(6)
    clear()
    os.system('./main.py')
